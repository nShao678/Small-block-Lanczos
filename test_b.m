...
sampleMax = 1000;
d = 4;
dataNormS = zeros(sampleMax,bMax);

dataKappaOmega = zeros(1,bMax);
for b = 2:bMax
    Omega = cell(1,d);
    m = b*d;
    ...
    for sample = 1:sampleMax
        ...
        dataNormS(sample,b) = normS^(1/(d-1))/m;
        dataKappaOmega(sample,b) = kappaOmega^(1/(d-1));
    end
end
...
hold off
set(gca,'yscale','log')
legend('FontSize',18,'Interpreter','latex','Location','east','Box','off')
set(gcf, 'Color', 'w');
export_fig('fig/EPb.pdf')
export_fig('fig/EPb.eps')
